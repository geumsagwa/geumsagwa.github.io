...
- **Git 정본:** 브랜치 **`master`**, **`origin/master` tip = `6089adc`** (인계·하네스 tip 정리 `progress.md`); 기능 변경 **`dd325d1`** (카카오 콜백·README).
- **레거시 브랜치:** `chore/public-readiness-20260312` 원격 tip **`4413071`** (변동 없음).
- **하네스 인계 미러:** `geumsagwa-harness` `main` tip **`89e3f82`** → `docs/desktop-handoff/` (`handover-progress.md`, `progress.md`).

## 브랜치·원격
- **작업 브랜치:** `master`
- 원격: `https://github.com/geumsagwa/geumsagwa.github.io.git`
- **최근 `master` 히스토리:** tip **`6089adc`** (progress·인계 동기); Kakao Edge·스크립트·직접 OAuth 누적은 **`dd325d1`** 등. 이전 **`5d792d9`**(SDK 제거·직접 OAuth code→token) 등은 히스토리 보존.
