...
- 시각(ISO): **`2026-08-01T14:30+09:00`** — **LLM Wiki low-confidence 전면 정리 (9,358 → 0).**
  - **[LLM Wiki 정리]** low-confidence **9,358 → 0개** 전면 해결 ✅
    ...
    - ① 스텁 승격: `promote-stubs.mjs` 신규 — 8,023개 confidence:low → medium (LLM 불필요)
    - ② 일반 low 재생성: limit 200·batch 20, 독립 프로세스로 ~1,000개 재생성
    - ③ 깨진 링크: 재생성 유입 최대 301종 → fix-wikilinks + create-stub-concepts → **0개**
...
- 시각(ISO): **`2026-08-01T08:30+09:00`** — **게발이 브리핑 발행 + 인계 자동 아카이브 규칙 + 툴 도입(claude-hud·주간 스캔).**
  ...
  - **Git:** homepage `3832b60` ✅, harness `740500c` ✅
