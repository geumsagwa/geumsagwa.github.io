...
- 시각(ISO): **`2026-09-16T22:58+09:00`** — **llm-wiki 원고 창: 도판 출처 줄(§8.5.8) 게이트 조문 확정 + CC 도판 소급 13화 21점 + 1·2·3권 7화 줄바꿈 정규화 + 물음표 통일분 재발행 9화 + 인계 갱신(6차).**
  - [자동수집 · Git] 마지막 세션(2026-09-16T22:20) 이후 — llm-wiki `b7ae1146`·`9f00f721`·`64775ba6`(origin `706f642c`) · homepage `1cb8898` · harness `52809cd` · openclaw `11a8f0a`(커밋 없음)
  - [지침 개정 · 무진님 결정] **지침 §8.5 사진 규칙 8 신설** — `> <span class="fig-credit">` 출처 줄은 **동어반복(내부·교차 word-12) 검사 대상에서 제외**(검사 전 본문에서 제거). 계기: 3권 29~33·35화가 같은 라이선스 표기 줄(`by-sa/4.0`… — 이름·URL이 고정 문자열)로 서로 10~15건 겹쳐 게이트 FAIL, **3권 34화는 내부 중복 12건을 안은 채 발행**돼 있었다. 게이트 공용 검사기 **`ws_tmp/gate_common.py`(19검사)** 신설 — 화별 `_gateNN.py`를 대신하며 **git 비추적**.
  - [CC 도판 소급 — 이월 과제 ②] **미발행 13화 21점에 §8.5.8 출처 줄 소급** — 3권 37~40화 · 4권 42·44·45·46화 · 5권 48·57·58·59·60화. 캡션(`> **그림N.**`) **바로 아래 줄**에 삽입(PD·CC0는 미표기) · 02 검토내용 13파일에 소급 기록 절 추가 · Commons API(`imageinfo/extmetadata`)로 저작자·라이선스 **전건 재확인** · 게이트 신규 FAIL 0 · 커밋 `64775ba6`(26파일 2,039+/35−). 비해당 확정: 1권 2·9·10화 · 2권 25화 · 4권 43·47화 · 5권 49~56화(채택본이 PD·CC0).
  - [줄바꿈 정규화 · 무진님 지시] 심리학사 **1·2·3권 7화 CRLF→LF**(1권 11·12·14화 · 2권 23·24화 · 3권 36·37화, CR 228~376→0, **내용 변경 0**) + 지침 §8.5 조문 — 커밋 `9f00f721`(8파일).
  - [재발행] **3권 29~33화**(ids 51~55 · 출처 줄 반영 · 커밋 `b7ae1146`) · **줄바꿈 정규화분 6화**(심리학사 11·12·14·23·24·36화 = ids 33·34·36·45·46·58, DB CR=0) · **물음표 통일분 9화**(심리학사 13·17·18·25·26화 = ids 35·39·40·47·48 / 철학사 1·4·8·10화 = ids 8·17·30·32).
  - [상태] **심리학사 발행분 = 제1~36화(ids 12~58)**뿐(제37화 이후 미발행 · 제36화부터 검토·발행은 **무진님 직접**). 철학사 = 제1~10화(ids 8~32). 5권은 제61화로 원고 마감(다음 = 6권 6-1 = 제62화).
  - [인계 갱신 · 6차] 확립 파이프라인 `update-handover.py collect → apply --git` 실행 — 아카이브(최근 5세션) 후에도 5세션 합계가 33,020B라 인계 게이트 FAIL → 자동수집 Git 블록을 구(舊)세션과 같은 1줄 형식으로 압축하고 초안 자리표시 줄 정리 → **29,506B · 게이트 PASS**(5미러 바이트 동일 · BOM 없음) → 커밋·푸시(homepage `d543e7b` · harness `dccdfe9`). 이어 **CC 도판 소급 수치 정정(20점 → 21점)** 커밋 homepage `18686a2` · harness `81bfc9b`. `F:\wiki\HANDOVER.md` 6차 노트·5·8·16·46·77행 갱신(`f704d205`·`013e2aed`·`3f1488ca`). **llm-wiki 저장소 7커밋도 푸시 완료**(`706f642c..3f1488ca`, 무진님 지시 — 파이프라인 자체는 homepage·harness만 푸시).
  - [보고 · 무진님 확인 대기] ① 커밋 `64775ba6`에 **종전 untracked 8파일**(3-38·4-45·5-59·5-60의 01·02)이 함께 실림 — "4·5권은 미커밋 유지" 관행과의 정합성 ② **중복 도판 2쌍**(3-40 그림1 = 3-30 그림3 · 4-44 그림3 = 4-45 그림4) ③ **기존 게이트 편차**(정의부 내부 빈 줄 · 정의 형식 · 해요체 · 3-40 문단 하드랩) ④ 1권 14화·철학사 구형 서식은 `---` 구분선이 없어 `gate_common.py` 중단 ⑤ **3-34화가 내부 동어반복 12건을 안은 채 발행**된 상태.
...
- 시각(ISO): **`2026-09-16T22:20+09:00`** — **제61화(5-14 「20세기 후반의 심리학 — 다양성과 통합」) 집필 완성 + 도판 6자리 무진님 승인.** 산출물 3종(01 본문·02 검토내용·도판의뢰서) 완성, **미커밋·미발행**(무진님 검토 대기). E-basis **14,519**(목표 14,400~15,100 · 16p) · 각주 **34**(§10.0 상한 재심사로 초안 38 → 34, 덜어낸 4건은 02 ⑤ 검증표로 이관) · 단어코너 3(학제·절충·다양성, 전부 다체) · 게이트 **23/23 PASS** · DAV **34/34**(인용 조각 57) · `candidates.tsv` 수치 43+인용 57 전부 FOUND(불일치 0, 미검증·보류 7 = 서술어 6 + 앞 권 검증 1). 본문 sha256 `8754d490…` · candidates sha256 `87635472…`. 도판: 실사 4점(그림3 스페리·그림4 벡·그림5 클라크 부부·그림6 APA 본부) 배치 완료 + 도해 2점(그림1 학파→분야·그림2 마음의 세 층) 규격 승인 — **여섯 자리 모두 무진님 승인(2026-09-16)**, 도해는 제작 후 같은 파일명으로 교체(01 본문 수정 불필요). 그림4는 밴두라(CC BY-SA 4.0) 후보가 3권 35화와 출처 줄이 겹쳐 게이트 교차 동어반복에 걸려 벡(PD)으로 확정. 다음 작업: **6권 6-1(제62화 「신경과학의 도약 — 마음의 지도」·18p)**.
  - [자동수집 · Git] 마지막 세션(2026-09-16T20:05) 이후 — llm-wiki `706f642c`(origin 동일) · homepage `69e415d` · harness `270f0b4` · openclaw `11a8f0a`(커밋 없음)
