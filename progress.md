...
## 다음에 할 일 (최대 4개)
1. (대기) 심리학사 1권 4화 무진님 검토 → 확정 시 검토내용 작성·홈페이지 업로드 (초고 완성 26,469자·각주 24개[^42]~[^65], llm-wiki `46342f40`) — 1-3화 ✅ 확정 (2026-08-20, 본문 17,217자·각주 23개)
...
- **LLM Wiki:** `59c892ce` — 심리학사 1-3화 확정 + 검토내용 + 분량표(17,217자) ✅ push 완료 (커밋된 범위 clean · **철학사 폴더 개편 등 타 변경은 워킹트리에 잔존**) (이전 `d31789cb`: 1-3화 초고 + 집필지침 §5/§6 · `47af67fb`: 1-2화 확정 · `bb8079ed`: 1-1화 확정+검토내용 · `db3d2230`: 개인 기록 확장)
- **homepage:** `a3fdf64` — 에세이 이미지 반응형(.review-body img max-width 100% + 캐시 버전 20260820) ✅ push 완료 (clean, 로컬=원격) (이전 `e848cbe`: .serena memories 제외 · `7556ab0`: 08-20 브리핑 카드뉴스 · `cf40fe0`: 틸드 취소선 보강 · `2bcf0d5`: 연재 발행 파이프라인 통합 · `2b95b62`: 심리학사 연재 등록 · `ba47069`: 08-19 브리핑 카드뉴스)
