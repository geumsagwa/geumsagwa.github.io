...
- 시각(ISO): **`2026-09-16T22:58+09:00`** — **llm-wiki 원고 창: 도판 출처 줄(§8.5.8)의 게이트 조문 확정 + CC 도판 소급 13화 20점 + 1·2·3권 7화 줄바꿈 정규화 + 물음표 통일분 재발행 9화.**
  - [자동수집 · Git] 마지막 세션(2026-09-16T22:20+09:00) 이후:
    · homepage master: HEAD 1cb8898 / origin 1cb8898 ·작업트리 변경 2 :: 1cb8898 진행 기록: 인계 갱신 (update-handover auto) / 69e415d 진행 기록: 인계 갱신 (update-handover auto) / 0a2533d 진행 기록: 인계 갱신 (update-handover auto) / 5a11afd 진행 기록: 인계 갱신 (update-handover auto) / 0fec457 자동: 카드뉴스 갱신 (2026-09-16)
    · llm-wiki master: HEAD 64775ba6 / origin 706f642c ·작업트리 변경 214 :: 64775ba6 심리학사 CC 도판 소급 — 13화 20점에 §8.5.8 출처 줄 적용 (이월 과제 ②) / 9f00f721 심리학사 1·2·3권 7화 줄바꿈 정규화(CRLF→LF) + 지침 §8.5 출처 줄 동어반복 검사 제외 조문 / b7ae1146 CC 도판 출처 줄(§8.5.8) 소급 — 철학사 1권 10화·심리학사 3권 29~33화 / ea47770c docs(handover): 61화 집필 완성 · 도판 6자리 승인 · §10.0 각주 재심사(38→34) / 706f642c 1권 4~10화 실사 도판(그림 1) 소급 배치 / eea262e2 1권 1~3화 실사 도판(그림 1) 소급 배치 / ebfd6e72 HANDOVER: 4차 갱신 — 1순위 4화 재발행 완료 + 1권 도해 캡션 소급 + 물음표
    · harness  main: HEAD 52809cd / origin 52809cd ·작업트리 변경 3 :: 52809cd docs(desktop-handoff): 인계 갱신 동기 (update-handover auto) / 270f0b4 docs(desktop-handoff): 인계 갱신 동기 (update-handover auto) / 6574f7a docs(desktop-handoff): 인계 갱신 동기 (update-handover auto) / 60a7e25 docs(desktop-handoff): 인계 갱신 동기 (update-handover auto)
    · openclaw main: HEAD 11a8f0a / origin 11a8f0a ·작업트리 변경 5 :: (커밋 없음)
  - [지침 개정 · 무진님 결정] **지침 §8.5 사진 규칙 8 신설** — `> <span class="fig-credit">`로 시작하는 **출처 줄은 동어반복(내부·교차 word-12) 검사 대상에서 제외**한다(검사 전 본문에서 제거). 계기: 3권 29~33·35화가 같은 라이선스 표기 줄(`by-sa/4.0`…, 라이선스 이름·URL이 **고정 문자열**)로 서로 10~15건 겹쳐 게이트에 걸렸고, **3권 34화는 같은 사유로 내부 중복 12건을 안은 채 발행**돼 있었다. 출처 줄은 문장이 아니라 법이 요구하는 인용 표기이므로 검사에서 뺀다. **게이트 공용 검사기 `ws_tmp/gate_common.py`(19검사)** 신설 — 화별 `_gateNN.py` 대신 재사용, **git 비추적**.
  - [CC 도판 소급 — 이월 과제 ②] **미발행 13화 20점에 §8.5.8 출처 줄 소급 적용** — 3권 37·38·39·40화 · 4권 42·44·45·46화 · 5권 48·57·58·59·60화. 캡션(`> **그림N.**`) **바로 아래 줄**에 `> <span class="fig-credit">출처 — …</span>` 삽입(PD·CC0는 미표기), 02 검토내용 13파일에 소급 기록 절 추가, Commons API(`imageinfo/extmetadata`)로 저작자·라이선스 **전건 재확인**, 삽입 전후 게이트 **신규 FAIL 0**. 커밋 `64775ba6`(26파일 2,039+/35−). CC 소급 비해당으로 확정: 1권 2·9·10화 · 2권 25화 · 4권 43·47화 · 5권 49~56화(채택본이 PD·CC0).
  - [재발행 · 출처 줄] 조문 적용 후 **3권 29~33화 재발행**(`publish-series-episodes.mjs psychology 29 30 31 32 33 --update`) → DB 확인 `fig-credit` id51 1건 · 52 1건 · 53 1건 · 54 1건 · 55 1건. 커밋 `b7ae1146`.
  - [줄바꿈 정규화 · 무진님 지시] 심리학사 **1·2·3권 7화 CRLF→LF**(1권 11·12·14화 · 2권 23·24화 · 3권 36·37화, CR 228~376 → 0, **내용 변경 0**) + 지침 §8.5 조문 → 커밋 `9f00f721`(8파일) → **재발행 6화**(`psychology 11 12 14 23 24 36 --update` = ids 33·34·36·45·46·58) → **DB CR=0 확인**.
  - [물음표 통일분 재발행] 4차 ④ 표기 변경분 **9화 재발행** — 심리학사 제13·17·18·25·26화(ids 35·39·40·47·48) + 철학사 1·4·8·10화(ids 8·17·30·32).
  - [상태] **심리학사 발행분 = 제1~36화(ids 12~58)** 뿐 — 제37화 이후는 미발행(제36화부터 **검토·발행은 무진님 직접**). 철학사 발행분 = 제1~10화(ids 8~32). 5권은 제61화로 원고 마감(6권 6-1 = 제62화 대기).
  - [보고 · 무진님 확인 대기] ① 커밋 `64775ba6`에 **종전 untracked였던 8파일**(3-38·4-45·5-59·5-60의 01·02)이 함께 실림 — "4·5권 원고는 대체로 미커밋 유지" 관행과의 정합성 확인 필요 ② **중복 도판 2쌍**(3-40 그림1 = 3-30 그림3 · 4-44 그림3 = 4-45 그림4, 같은 Commons 파일) ③ **기존 게이트 편차**(각주 정의부 내부 빈 줄 · 정의 형식 `｜ 근거: … 확인.` · 해요체 · 3-40 문단 하드랩·`>` 블록 여백) ④ 1권 14화·철학사 구형 서식은 `---` 구분선이 없어 `gate_common.py`가 중단 ⑤ **3-34화는 내부 동어반복 12건을 안은 채 발행된 상태**.
  - [다음 작업] **6권 6-1(제62화 「신경과학의 도약 — 마음의 지도」·18p)** 집필. 잔여 미커밋(문체 다듬기 · 캡션라벨 sweep · 5권 57~60화 · 세계사 파일군)은 무진님 판단 대기. 4~10화 판단 5건은 현 상태 유지.
...
- 시각(ISO): **`2026-09-16T20:05+09:00`** — **llm-wiki 원고 창: 3차 ⚠️(4화 재발행) 이행 + 1권 도해 안 캡션 소급(무진 승인) + 2권 27화 그림4 크롭 + 소제목 물음표 통일(무진 지시) + 1권 10화 02 검토내용 소실 복구(공개).**
  ...
  - [사고·복구(공개)] `manuscripts/psychology/volume1/1권_10화_02_검토내용.md`가 정규화 스크립트 인코딩 버그(`open(p,'wb')` 후 `TypeError`)로 0바이트 소실 → git HEAD 판본에서 재구성(캡션라벨 3·노트 3, 꼬리 바이트 HEAD 동일). 다른 피해 0(0바이트 파일 전수 확인).
  - [다음 작업] `HANDOVER.md` 4차 노트 커밋·푸시(llm-wiki repo, 이 파이프라인과 별개) · 승인 대기 = 물음표 통일분 재발행(심리학사 1권 13 / 2권 17·18·25·26화 / 철학사 1·4·8·10화) · 3차 ⚠️ CC 도판 소급 적용. 잔여 미커밋(문체 다듬기·캡션라벨 sweep·5권 57~60화·세계사 파일군)은 무진님 판단 대기.
