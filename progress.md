...
- 시각(ISO): **`2026-09-24T19:39+09:00`** — **심리학사 1권 14화·2권 15~19화 제작 도판 16종 배치 + 재발행(무진님 「한 번에」)**: 발행본 이미지 sha256 대조로 '미배치' 확정(16장 전부 placeholder) → 본문 정규화 → 커밋 `9be1fa92`(28건) → 재발행(ids 36~41) → 발행본 23장 전수 최종본 일치(PLACEHOLDER 0) · 게이트 6화 PASS 22/22.
  - [자동수집 · Git] 마지막 세션(2026-09-24T19:20+09:00) 이후:
    · homepage master: HEAD d6f38e9 / origin d6f38e9 ·작업트리 변경 5 :: d6f38e9 진행 기록: 인계 갱신 30KB 게이트 대응 — Git 상세 블록 4개 압축(33578B→30442B, PASS) / a6c37af 진행 기록: 인계 갱신 (update-handover auto) / 472046e SERIES_MAP에 psychology 44·45화 등록 — 4권 44·45화 발행 반영 / 9497ab3 진행 기록: 인계 갱신 (update-handover auto) / 8026707 자동: 카드뉴스 갱신 (2026-09-24)
    · llm-wiki master: HEAD 1581432a / origin 1581432a ·작업트리 변경 147 :: 1581432a HANDOVER: 44차 기록 — 14~19화 제작 도판 16종 배치·재발행 / 9be1fa92 도판 정규화 — 14~19화 제작 도해 16종 배치 (본문 슬롯 연결) / b421d0d0 6권 69화 집필 완료 — 「재현성 위기 — 과학의 자기 검증」(6-8·18쪽) + 인계 기록 보정 / 29c0bdba 42차 잔여 산출물 커밋 — 표기 정리 · 도판 2건 · 최종목차 자수 정리 / 89d67ef6 6권 68화 도판 확정 — 실사 4점 배치·CC 출처 줄 2 + 도해 2점 검증 확정
    · harness  main: HEAD b5b1acc / origin b5b1acc ·작업트리 변경 3 :: b5b1acc docs(desktop-handoff): 인계 갱신 항목 압축 — Git 상세 블록 4개(30KB 게이트 PASS, 30442B) / 3c16348 docs(desktop-handoff): 인계 갱신 동기 (update-handover auto) / fa54e8b docs(desktop-handoff): 인계 갱신 동기 (update-handover auto)
    · openclaw main: HEAD 11a8f0a / origin 11a8f0a ·작업트리 변경 5 :: (커밋 없음)
  - **결정·상태**: ① **반증** — 무진님 「14~19화 제작 도해는 재발행되었을텐데」는 **사실이 아니었음**: Supabase `essays` 14~19화 `body_markdown` 인라인 이미지 23장을 base64 디코드·sha256 대조 → 배치 전 **16장이 로컬 `_placeholder.png`와 정확히 일치**(사이트가 계속 placeholder 서비스 중). 원인 3중 = ⓐ 발행(2026-08-31~09-04) < 도해 제작(2026-09-22) ⓑ 제작 PNG 16종 git 미추적 ⓒ 본문이 `_placeholder.png` 참조. ② **정규화** — 6화 도판 슬롯 파일명 `_placeholder` 제거(16) · 캡션 끝 `(자리 표시 — AI 제작 예정)`·`(개념 도식 — AI 제작 예정)` 제거(13, 09-22 지시문 규정) · **1권 14화 §8.3 캡션 3건 신설 + alt 축약**(종전 캡션 부재로 FAIL이던 분 — 02 검토내용 80행 명시 대상) · 02 검토내용 6종에 '배치 완료(2026-09-24)' 기록 삽입. ③ **커밋·재발행** — llm-wiki `9be1fa92`(28건 = 제작 PNG 16 + 본문 6 + 02 6) push · HANDOVER `1581432a` · `publish-series-episodes.mjs psychology 14 15 16 17 18 19 --update` → 6화 갱신(id 36~41). ④ **검증** — 발행본 이미지 23장 전수 로컬 최종본 sha256 일치(PLACEHOLDER 0·UNKNOWN 0) · 게이트 `gate_common.py` 6화 **PASS 22/22**(E-basis 23,933/24,118/25,485/19,050/21,927/26,806 = 서사·각주·분량 불변). 출력 `ws_tmp/fig-repub/_gate_final.txt`·`_verify_pub.cjs`. ⑤ **주의 2건** — Supabase `updated_at`은 PATCH로 갱신 안 됨(발행 시각 판별 부적합) · **`_site_diff.cjs`는 이미지를 `IMG`로 정규화해 도판 교체류 차이를 원리적으로 감지 못 함**(diff PASS ≠ 도판 반영).
  - **다음 작업**: ⓐ **집필 잔여 = 6권 6-9~6-14(제70~75화)** — 다음 화 **6-9 = 제70화**(검증 파이프라인은 62~69화와 동일) · 컨텍스트 캐시 규약상 **새 창 권장**. ⓑ **무진님 몫 대기** — 65~69화 도판 실사 선택·도해 제작 의뢰·검토 확정(발행은 명시 지시 시에만). ⓒ 선택 — 14~19화 슬롯의 구 `_placeholder.png`(16종, 이제 미참조) 정리 여부 판단.
...
- 시각(ISO): **`2026-09-21T21:47+09:00`** — **각주 재편 B11-3 결재·발행 마감 — ⑴ 9·10화 `## 각주` 소제목 유지 ⑵ 9화 01 선두 작업 메모 1행 삭제 ⑶ 10화 도판 alt 최소 교정 승인 ⑷ 철학사 1권 1~10화 묶음 재발행 완료(사이트-원고 10/10 일치) — F단계 정규화분 전량 반영**
  - [자동수집 · Git] 마지막 세션(2026-09-21T21:23+09:00) 이후: HEAD — homepage a244bd9(wt 5) / llm-wiki 568dd1e8(wt 126) / harness de77a7f(wt 3) / openclaw 11a8f0a(wt 5) (커밋 상세는 각 repo `git log`)  - **결정**: 무진님 「１。 소제목 유지、 그런데 １행에 포함되어서는 안될 내용이 있음 -> 삭제 ２。 승인 및 발행」. 9·10화 `## 각주` 소제목은 **유지**(A안 — 1~8화와의 레이아웃 차이는 보존). 9화 01 머리에 남아 있던 작업 메모 1행 `43화  전체 재검증  -> 이상 없으면 확정 -> 검토내용 파일 갱신 -> 홈페이지 업로드`는 **삭제** — 발행 스크립트가 원고 파일 **전체**를 `body_markdown`으로 업로드하므로 그대로 두면 공개 본문 **첫 줄**로 노출될 상태였다(현재 공개본이 `# 제9화 …`로 시작하는 것은 재발행 전 구본이기 때문). 삭제 결과는 **커밋본과 byte-identical**(작업트리 잔여분 · `git diff` 없음) — **E-basis 31,395 불변** · 게이트 **20/20 PASS** 재확인. 10화 도판 그림 1 alt 최소 교정은 **승인**(A안 · ⑤ 고아 마커 교정도 확인).
